...
void Session::handle_read_body(const boost::system::error_code &ec, std::size_t bytes_transferred) {
	if (!ec){
		...
		client_decrypt(receive_buffer_.get(), actual_bytes_recv, &recv_buffer, &newSize, key_);

		receive_buffer_.reset(new unsigned char[newSize]());
		memcpy(receive_buffer_.get(), recv_buffer, newSize);

		pc_->handle_packet(newSize);
...
void Session::send_packet(Packet *packet) {
	...
	server_encrypt(packet_buffer, packet_length, &send_buffer, &newSize, key_);
