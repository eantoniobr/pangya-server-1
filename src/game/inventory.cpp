...
char pc_additem(pc *pc, item *item, bool test_additem, ITEM_TRANSACTION* tran) {
	...
	stm << "EXEC sys_add_item ?, ?, ?, 1", use(pc->account_id_), use(item->type_id), use(item->amount), now;
	RecordSet rs(stm);
...
void pc_savedata(pc *pc) {
	...
	sql_string.append(boost::str(boost::format("UPDATE statistic SET pang = %1% WHERE account_id = %2%;")
		% pc->state->pang % pc->account_id_));
	count++;

	sql_string.append(boost::str(boost::format("UPDATE account SET cookie = %1% WHERE account_id = %2%;")
		% pc->cookie % pc->account_id_));
	count++;
