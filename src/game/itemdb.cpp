...
void ItemDB::readdb_char() {
	std::fstream f;
	int count;

	f.open("db/Character.iff", std::fstream::in | std::fstream::binary);
	f.read((char*)&count, sizeof(count));
	f.seekg(4, std::ios::cur);

	for (int i = 1; i <= count; ++i) {
		std::unique_ptr<itemdb_char> it = CREATE_UNIQUE(itemdb_char);
		f.read((char*)& it->base.enable, sizeof(itemdb_char));
		char_data[it->base.item_typeid] = std::move(it);
	}
	f.close();
}
...
bool ItemDB::exists(uint32 id) {
	...
	if (item_type == ITEMDB_CHAR) {
		if (char_data.find(id) != char_data.end()) {
			return true;
		}
	}
	else if (item_type == ITEMDB_USE) {
...
std::pair<char, uint32> ItemDB::get_price(uint32 id) {
	...
	if (item_type == ITEMDB_CHAR) {
		if (char_data.find(id) != char_data.end()) {
			return std::make_pair(char_data[id]->base.price_type, char_data[id]->base.discount_price > 0 ? char_data[id]->base.discount_price : char_data[id]->base.price);
		}
	}
	else if (item_type == ITEMDB_USE) {
		if (item_data.find(id) != item_data.end()) {
		...
		if (part_data.find(id) != part_data.end()) {
			return std::make_pair(part_data[id]->base.price_type, part_data[id]->base.discount_price > 0 ? part_data[id]->base.discount_price : part_data[id]->base.price);
		}
	}
	else if (item_type == ITEMDB_CLUB) {
		if (club_data.find(id) != club_data.end()) {
			return std::make_pair(club_data[id]->base.price_type, club_data[id]->base.discount_price > 0 ? club_data[id]->base.discount_price : club_data[id]->base.price);
		}
	}
	else if (item_type == ITEMDB_BALL) {
		if (ball_data.find(id) != ball_data.end()) {
			return std::make_pair(ball_data[id]->base.price_type, ball_data[id]->base.discount_price > 0 ? ball_data[id]->base.discount_price : ball_data[id]->base.price);
		}
	}
	else if (item_type == ITEMDB_CARD) {
		if (card_data.find(id) != card_data.end()) {
...
bool ItemDB::buyable(uint32 id) {
	...
	if (item_type == ITEMDB_CHAR) {
		if (char_data.find(id) != char_data.end()) {
			return char_data[id]->base.flag & 1;
		}
	}
	else if (item_type == ITEMDB_USE) {
		if (item_data.find(id) != item_data.end()) {
		...
		if (part_data.find(id) != part_data.end()) {
			return part_data[id]->base.flag & 1;
		}
	}
	else if (item_type == ITEMDB_CLUB) {
		if (club_data.find(id) != club_data.end()) {
			return club_data[id]->base.flag & 1;
		}
	}
	else if (item_type == ITEMDB_BALL) {
		if (ball_data.find(id) != ball_data.end()) {
			return ball_data[id]->base.flag & 1;
		}
	}
	else if (item_type == ITEMDB_CARD) {
		if (card_data.find(id) != card_data.end()) {
