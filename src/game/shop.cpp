...
void pc_buyitem_normal(pc* pc) {
	...
	if (!pc->removepang(pang_total) || !pc->removecookie(cookie_total))
	{
		send_msg(pc, BUY_FAIL);
		return;
	}
...
void buyitem_result(pc* pc, ITEM_TRANSACTION* tran, uint16 day, uint8 flag) {
	...
	WTIU64(&p, pc->state->pang); // pc pang
	WTIU64(&p, pc->cookie); // pc cookie;
	pc->send(&p);
}
...
void send_msg(pc* pc, uint32 code, bool success) {
	...
	if (success) {
		p.write<uint64>(pc->state->pang);
		p.write<uint64>(pc->cookie);
	}
