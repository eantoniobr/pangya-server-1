...
typedef std::shared_ptr<Achievement> PC_ACHIEVEMENT;
...
typedef std::shared_ptr<Counter> PC_COUNTER;

struct Quest
{
	uint32 id;
	uint32 achievement_id;
	uint32 achievement_quest_typeid;
	uint32 counter_id;
	Poco::DateTime success_date;
	uint32 count;
};
typedef std::shared_ptr<Quest> PC_QUEST;
...
struct pc {
	...
	// Achievement
	std::vector<PC_ACHIEVEMENT> achievements;
	std::unordered_map<uint32, PC_COUNTER> counters;
	std::vector<PC_QUEST> quests;
